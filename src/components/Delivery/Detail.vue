<template>
  <div>
   <el-dialog class="AddDialog" title="送货单详情" :visible.sync="addDialogVisible" fullscreen @close="close">
      <!-- topInfo -->
...
export default {
  ...
  methods: {
    ...
    close () {
      this.$emit('toggleDetailDialog', false)
    },
    Submit (formName) {
    ...
    getDetail () {
      this.Http.get('shorderdetail', {id: this.curSongHuoId}
      ).then(res => {
        if (res.data.code === 1) {
          ...
          this.Form = {
            ...
            supplierShortName: topInfo.providername,
            providercode: topInfo.providercode,
            contacts: topInfo.providerproxy,
            tel: topInfo.tel,
            fax: topInfo.fax,
